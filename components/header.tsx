...
export default function Header() {
    const { loggedUser, loggedIn } = useAuth();
    return (
        <>
            <header className="text-gray-600 body-font">
                <div className="container mx-auto flex flex-wrap p-5 flex-col md:flex-row items-center">
                    ...
                    >
                        <>
                            {loggedIn ? (
                                <img src={loggedUser?.user_metadata.avatar_url} className="w-10 h-10 text-white rounded-full" />
                            ) : (
                                    <svg
                        ...
                        <span className="ml-3 subpixel-antialiased text-xl dark:text-gray-400">
                            Devlog {loggedIn && `<${loggedUser?.user_metadata.preferred_username}/>`}
                        </span>
                    </Link>
